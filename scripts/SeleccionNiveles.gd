...
var ruta_niveles_json: String = "res://data/niveles.json"
...
func _ready() -> void:
	...
	if SaveManager and SaveManager.nivel_actual_seleccionado > 0:
		pagina_actual = int(ceil(float(SaveManager.nivel_actual_seleccionado) / float(NIVELES_POR_PAGINA)))
		if pagina_actual < 1:
			pagina_actual = 1
